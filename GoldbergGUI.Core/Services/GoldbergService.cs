...
namespace GoldbergGUI.Core.Services
{
    ...
    {
        private readonly ILogger<GoldbergService> _log;
        ...
        public GoldbergService(ILogger<GoldbergService> log)
        {
            _log = log;
        }
        ...
        private async Task StartDownload(string downloadUrl)
        {
            ...
            {
                ShowErrorMessage();
                _log.LogError(e, "Error during download");
                Environment.Exit(1);
            }
        }
            ...
            using (var archive = await Task.Run(() => ZipFile.OpenRead(archivePath)).ConfigureAwait(false))
            {
                foreach (var entry in archive.Entries)
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            errorOccured = true;
                            _log.LogError(e, "Error while trying to extract {FullName}", entry.FullName);
                        }
                    }).ConfigureAwait(false);
                }
            }
